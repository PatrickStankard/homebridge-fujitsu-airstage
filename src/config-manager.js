...
class ConfigManager {
    constructor(config, api) {
        ...
        this.persistFile = path.join(
            ...
            "airstage-tokens.json",
        );
        this.temperatureScaleFile = path.join(
            this.api.user.persistPath(),
            "airstage-temperature-scales.json",
        );
    }
        ...
        try {
            ...
            if (this.api && this.api.logger) {
                this.api.logger.error(
                    "Failed to write Airstage tokens to persistPath:",
                    err.message,
                );
            }
        }
    ...
    getTokens() {
        ...
        try {
            if (fs.existsSync(this.persistFile)) {
                ...
                tokens.accessTokenExpiry = tokensData?.accessTokenExpiry
                    ? new Date(tokensData.accessTokenExpiry)
                    : null;
            ...
            if (this.api && this.api.logger) {
                this.api.logger.error(
                    "Failed to read Airstage tokens from persistPath:",
                    err.message,
                );
            }
        }
    ...
    migrateTokensFromConfig() {
        ...
        const tokens = {
            ...
            accessTokenExpiry: this.config.accessTokenExpiry
                ? new Date(this.config.accessTokenExpiry)
                : null,
    ...
    saveTemperatureScale(deviceId, scale) {
        try {
            ...
            if (this.api && this.api.logger) {
                this.api.logger.error(
                    "Failed to write temperature scales to persistPath:",
                    err.message,
                );
            }
        }
    ...
    getTemperatureScales() {
        try {
            ...
            if (this.api && this.api.logger) {
                this.api.logger.error(
                    "Failed to read temperature scales from persistPath:",
                    err.message,
                );
            }
        }
