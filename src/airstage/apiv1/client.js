...
class Client {
    ...
    // GET /apiv1/users/me
    getUsersMe(callback) {
        this._makeGetRequest(constants.PATH_USERS_ME, null, callback);
    }
    ...
    putUsersMe(parameterName, parameterValue, callback) {
        ...
        this._makePutRequest(constants.PATH_USERS_ME, requestBody, callback);
    }
    ...
    getDevicesAll(limit, callback) {
        ...
        this._makeGetRequest(constants.PATH_DEVICES_ALL, params, callback);
    }
    ...
    getDevice(deviceId, callback) {
        const path = constants.PATH_DEVICES_ONE.replace("%1", deviceId);

        this._makeGetRequest(path, null, callback);
    }
    ...
    postDevicesSetParametersRequest(
        ...
        const path = constants.PATH_DEVICES_SET_PARAMETERS_REQUEST.replace(
            "%1",
            deviceId,
        );
        const requestBody = {
        ...
        this._makePostRequest(path, requestBody, callback);
    }

    // GET /apiv1/devices/[deviceId]/requests
    getDevicesRequests(deviceId, callback) {
        const path = constants.PATH_DEVICES_REQUESTS_ALL.replace(
            "%1",
            deviceId,
        );
    ...
    getDevicesRequest(deviceId, requestId, callback) {
        const path = constants.PATH_DEVICES_REQUESTS_ONE.replace(
            "%1",
            deviceId,
        ).replace("%2", requestId);
    ...
    // GET /apiv1/groups
    getGroups(callback) {
        this._makeGetRequest(constants.PATH_GROUPS, null, callback);
    }
    ...
    _makeGetRequest(path, params, callback) {
        ...
        if (this.accessToken) {
            requestHeaders[constants.REQUEST_HEADER_AUTHORIZATION] =
    ...
    _makeRequestWithRequestBody(method, path, requestBody, callback) {
        ...
        let request = null;
        let requestOptions = null;
        let requestHeaders = structuredClone(
            constants.REQUEST_HEADERS_POST_PUT,
        );

        requestHeaders[constants.REQUEST_HEADER_USER_AGENT] = this.userAgent;
        requestHeaders[constants.REQUEST_HEADER_CONTENT_LENGTH] =
            requestBodyJson.length;
    ...
    _makeHttpsRequest(requestOptions, requestBodyJson, callback) {
        ...
        if (hostname === null) {
            result.error =
                "Could not determine hostname for region: " + this.region;
