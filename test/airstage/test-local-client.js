...
test("LocalClient#getTemperatureScale returns stored scale from configManager", (context, done) => {
    ...
    client.getTemperatureScale((error, scale) => {
        ...
        assert.strictEqual(
            mockConfigManager.getTemperatureScale.mock.calls.length,
            1,
        );
        done();
    });
...
test("LocalClient#setTemperatureScale saves scale for all devices", (context, done) => {
    ...
    client.setTemperatureScale(
        ...
        (error, scale) => {
            ...
            assert.strictEqual(
                mockConfigManager.saveTemperatureScale.mock.calls.length,
                2,
            );
            assert.strictEqual(
                mockConfigManager.saveTemperatureScale.mock.calls[0]
                    .arguments[0],
                "A0B1C2D3E4F5",
            );
            assert.strictEqual(
            ...
            assert.strictEqual(
                mockConfigManager.saveTemperatureScale.mock.calls[1]
                    .arguments[0],
                "F5E4D3C2B1A0",
            );
            assert.strictEqual(
